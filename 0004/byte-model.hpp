...
Byte_Model::Byte_Model(const std::vector<bool>& vocab) : ex(0),top_(255), mid_(0),
    bot_(0),  vocab_(vocab), probs_(1.0 / 256, 256) {}

 std::valarray<float>& Byte_Model::Predict()  {
...
const std::valarray<float>& Byte_Model::BytePredict() {
  return probs_;
}

void Byte_Model::Perceive(int bit) {
