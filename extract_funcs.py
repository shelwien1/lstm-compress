...
def parse_ast_dump(astdump_file, target_source):
    ...
    # Format: |-FunctionDecl 0xADDR <location> col:N [used] function_name 'type'
    func_decl_pattern = re.compile(r'\|-?FunctionDecl\s+\S+\s+<[^>]+>\s+\S+\s+(?:used\s+)?(\w+)\s+')
        ...
        with open(astdump_file, 'r', encoding='utf-8', errors='replace') as f:
            for line in f:
                ...
                if loc_match:
                    ...
                    if parts[0] and not parts[0].startswith('line') and not parts[0].startswith('col'):
                        # This is a filename
